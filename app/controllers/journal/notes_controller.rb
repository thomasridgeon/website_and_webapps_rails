class Journal::NotesController < ApplicationController
  ...
  def show
    ...
    render html: Journal::Note.new(plaintext: @plaintext, note_id: @note_id, controller: self).to_html.html_safe
  end
  ...
  def update
    ...
    redirect "/journal/notes/#{@note.id}" # after updating, redirect the user to the show page for that note (GET /journal/notes/:id)
  end
  
  def destroy
    @note.destroy
    redirect "/journal/notes", notice: "Note was successfully deleted."
  end

  private 

  def require_user
    redirect "/journal/login" unless current_user
  end
