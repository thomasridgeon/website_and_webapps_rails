  ...
  def page(title, &block)
    html do
      ...
      body(class: "min-h-screen p-4 flex flex-col items-center") do
        # Nav row
        div(class: "w-full max-w-3xl flex items-center justify-end mb-6") do
          div(class: "flex items-center space-x-4") do
            a(href: "/journal/notes", class: "text-sm text-gray-700 hover:underline") { text "Notes" }
            if @controller && @controller.respond_to?(:current_user) && @controller.current_user
              ...
              form(action: "/journal/logout", method: "post", class: "flex items-center m-0 p-0") do
                input(type: "hidden", name: "authenticity_token", value: @controller.send(:form_authenticity_token))
                input(type: "hidden", name: "_method", value: "delete")
                input(type: "submit", value: "Logout", class: "text-sm text-gray-700 hover:underline bg-transparent border-0 cursor-pointer")
              end
            else
